...
internal sealed partial class GameModel
{
    ...
    {
        return new string[] { "ヴェール", "ヴァイン", "ニトロ", "オアシス", "ディバイド", "グリッチ" };
    }

    private static string[] StructureSkinCatalog()
    {
        return new string[] { "シグナル標準", "カーボンゲート", "サンドパルス", "プリズムバイザー", "ローグクローム" };
    }

    private static string[] AdThemeCatalog()
    {
        return new string[] { "NEO CORE", "VERTEX CUP", "SUNSET GRID", "ARC LEAGUE" };
    }

    private static string[] BannerCatalog()
    {
        return new string[] { "SIGNAL//STANDARD", "CONTRACT//ARC", "BOSS//BACKER", "MAP//ARCHITECT", "AD//PARTNER" };
    }

    private static string[] KillEffectCatalog()
    {
        return new string[] { "SIGNAL BURST", "RIPPLE TRACE", "PRISM BREAK", "CLEAN CUT" };
    }
