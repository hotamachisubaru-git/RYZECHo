...
internal sealed partial class GameModel
{
    ...
    private void DrawCombatBottomBar(Graphics graphics)
    {
        ...
        DrawInsetPanel(graphics, statusRect);
        DrawHudText(graphics, _player.IsBoss ? "BOSS" : "RYZE", 8.4f, FontStyle.Bold, PhaseColor(), statusRect.Left + 8, statusRect.Top + 5);
        ...
        DrawAbilitySlot(graphics, skillRects[0], _player.Weapon == _playerPrimaryWeapon ? "Q" : "E", weapon.ShortLabel, weapon.VisionClass, false, WeaponAccent(weapon.Type), 1, Math.Clamp(weapon.VisionRange / 500f, 0f, 1f), true);
        DrawAbilitySlot(graphics, skillRects[1], "W", "MAG", $"{weapon.MagazineAmmo}/{weapon.ReserveAmmo}", false, Color.FromArgb(255, 116, 212, 230), 1, fireCharge, fireCharge >= 0.995f);
        DrawAbilitySlot(graphics, skillRects[2], "E", "ULT", _player.IsBoss ? $"K{_roundBossKillCount}" : "LOCK", _player.IsBoss, Color.FromArgb(255, 164, 220, 116), _player.IsBoss ? Math.Max(1, SelectedBossUltPoints()) : 0, BossInvestmentProgress(CurrentBossInvestment(_player)), _player.IsBoss);
